...
for i in range(0, num_blocks): # Until end of Song 
    ...
    for event in events:
        if event.type == pygame.KEYDOWN:
            ...
            elif event.key == pygame.K_v: condition2True = 1
            elif event.key == pygame.K_e: condition3True = 1
            elif event.key == pygame.K_f: condition4True = 1
            elif event.key == pygame.K_r: condition5True = 1
        if event.type == pygame.KEYUP:
            if event.key == pygame.K_a: condition1True = 0
            elif event.key == pygame.K_v: condition2True = 0
            elif event.key == pygame.K_e: condition3True = 0
            elif event.key == pygame.K_f: condition4True = 0
            elif event.key == pygame.K_r: condition5True = 0
    ...
    if (zG > 4 or condition1True):
        ...
        output_block = func_duck(input_tuple, RATE, duckFreq)  
       
    
    #Condition 2    --for-->        VIBRATO
    elif (zGyro > .5 or condition2True):
        vibratoFreq = 10            
        vibratoAmplitude = 0.6      # Keep < 1
        output_block = func_vibrato(input_tuple, RATE, vibratoFreq, vibratoAmplitude)
        
    
    #Condition 3    --for-->        ECHO
    elif (zGyro > .8 or condition3True):
        Gdp = 1            
        Gff = 2      
        echo = int(buffer_MAX/2)  #1/2 of the distance of its buffer
        output_block = func_echo(input_tuple, Gdp, Gff, echo)
        
    
    #Condition 4    --for-->        FEEDBACK
    elif (yGyro > .8 or condition4True):
        Gdp = 1            
        Gff = 2      
        Gfb = 0.4
        output_block = func_feedback(input_tuple, Gdp, Gff, Gfb)
